...
export default function AddMetricToPlanForm({
    ...
    return (
        <form onSubmit={onSubmit} className="mt-3 flex flex-col space-y-2 border-t pt-3">
            <h4 className="text-sm font-medium">Attach Metric</h4>

            {error && <p className="text-sm text-red-600">{error}</p>}

            <select name="metricId" disabled={loading} required className="border border-gray-500 p-2 px-2 disabled:opacity-50">
                <option value="" className="bg-background">Select metric</option>
                {metrics.map((m) => (
