...
export default function AnalyticsEmptyState({ orgId }: { orgId: string }) {
  return (
      ...
      <div className="relative grid gap-4 lg:grid-cols-[1.2fr_1fr]">
        <div>
          ...
          <p className="mt-1 max-w-xl text-sm text-slate-600">
            Analytics will populate after your application sends meter events
            tied to tracked metrics.
          </p>
          <div className="mt-4 flex flex-wrap items-center gap-2">
            <Button asChild>
              <Link href={`/app/${orgId}/metrics`}>Configure metrics</Link>
            </Button>
        ...
        <div className="space-y-3">
          <HintCard
            icon={<Database className="size-4 text-cyan-600" />}
            title="Emit usage events"
            description="Post metric usage from your app or backend workers."
          />
          <HintCard
            icon={<Sparkles className="size-4 text-sky-600" />}
            title="Wait for aggregation"
            description="Totals update automatically as ingestion pipelines process events."
          />
        </div>
      </div>
    </section>
