...
export default function ApiKeysEmptyState({ orgId }: { orgId: string }) {
  return (
      ...
      <div className="relative grid gap-4 lg:grid-cols-[1.2fr_1fr]">
        <div>
          ...
          <p className="mt-1 max-w-xl text-sm text-slate-200">
            Create an API key to let your application send authenticated usage
            events.
          </p>
        ...
        <div className="space-y-3">
          <HintCard
            icon={<Shield className="size-4 text-cyan-600" />}
            title="Store keys securely"
            description="Save keys in backend env vars and never expose raw values in clients."
          />
          <HintCard
            icon={<Sparkles className="size-4 text-sky-600" />}
            title="Rotate regularly"
            description="Create new keys and revoke old ones to reduce credential risk."
          />
        </div>
      </div>
    </section>
