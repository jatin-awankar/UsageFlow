...
INSERT INTO "User" (id, email, password) VALUES
  ...
  ('single-org-owner', 'single@example.test', :'password_hash'),
...
INSERT INTO "Membership" (id, role, "userId", "orgId") VALUES
  ...
  ('single-owner-a', 'OWNER', 'single-org-owner', 'org-a'),
...
DO $$
BEGIN
  IF (SELECT count(*) FROM "Customer" WHERE "externalId" = 'shared-id') <> 2
     OR (SELECT count(DISTINCT id) FROM "Customer" WHERE "externalId" = 'shared-id') <> 2
     OR (SELECT count(DISTINCT "orgId") FROM "Customer" WHERE "externalId" = 'shared-id') <> 2 THEN
    RAISE EXCEPTION 'shared external IDs did not create separate Organization Customers';
  END IF;
  IF EXISTS (SELECT 1 FROM "Customer" WHERE "externalId" IN (' padded ', ' ', 'forbidden-id')) THEN
    RAISE EXCEPTION 'rejected creation committed a Customer';
  END IF;
END $$;
SQL
