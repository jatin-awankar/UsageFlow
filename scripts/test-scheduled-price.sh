...
set -euo pipefail
cd "$(dirname "$0")/.."
container="usageflow-scheduled-price-$$"
app_log="$(mktemp)"
cleanup() {
...
export NEXTAUTH_URL="http://127.0.0.1:3109"
export PRICE_TEST_BASE_URL="$NEXTAUTH_URL"
npx next dev -p 3109 >"$app_log" 2>&1 &
app_pid=$!
...
npx playwright test tests/scheduled-price.spec.ts --workers=1
...
DO $$ BEGIN
  ...
  IF (SELECT count(*) FROM "PriceVersion" WHERE "orgId" = 'org-a') <> 3 THEN RAISE EXCEPTION 'published count changed'; END IF;
END $$;
DO $$ DECLARE
  t1 timestamptz;
  t2 timestamptz;
  t3 timestamptz;
BEGIN
  SELECT "effectiveFrom" INTO t1 FROM "PriceVersion" WHERE "metricId" = 'metric-a' ORDER BY "effectiveFrom" LIMIT 1;
  SELECT "effectiveFrom" INTO t2 FROM "PriceVersion" WHERE "metricId" = 'metric-a' ORDER BY "effectiveFrom" OFFSET 1 LIMIT 1;
  SELECT "effectiveFrom" INTO t3 FROM "PriceVersion" WHERE "metricId" = 'metric-a' ORDER BY "effectiveFrom" OFFSET 2 LIMIT 1;
  IF (SELECT "unitPriceMicros" FROM "PriceVersion" WHERE "orgId" = 'org-a' AND "metricId" = 'metric-a' AND "effectiveFrom" <= t2 - interval '1 millisecond' ORDER BY "effectiveFrom" DESC LIMIT 1) <> 1000000 THEN RAISE EXCEPTION 'before boundary'; END IF;
  IF (SELECT "unitPriceMicros" FROM "PriceVersion" WHERE "orgId" = 'org-a' AND "metricId" = 'metric-a' AND "effectiveFrom" <= t2 ORDER BY "effectiveFrom" DESC LIMIT 1) <> 2000000 THEN RAISE EXCEPTION 'at boundary'; END IF;
  IF (SELECT "unitPriceMicros" FROM "PriceVersion" WHERE "orgId" = 'org-a' AND "metricId" = 'metric-a' AND "effectiveFrom" <= t2 + interval '1 millisecond' ORDER BY "effectiveFrom" DESC LIMIT 1) <> 2000000 THEN RAISE EXCEPTION 'after boundary'; END IF;
  IF (SELECT "unitPriceMicros" FROM "PriceVersion" WHERE "orgId" = 'org-a' AND "metricId" = 'metric-a' AND "effectiveFrom" <= t3 ORDER BY "effectiveFrom" DESC LIMIT 1) <> 3000000 THEN RAISE EXCEPTION 'third boundary'; END IF;
END $$;
DO $$ BEGIN
