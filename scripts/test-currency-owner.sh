...
set -euo pipefail
cd "$(dirname "$0")/.."
container="usageflow-currency-owner-$$"
app_log="$(mktemp)"
cleanup() {
...
export NEXTAUTH_URL="http://127.0.0.1:3107"
export CURRENCY_TEST_BASE_URL="$NEXTAUTH_URL"
npx next dev -p 3107 >"$app_log" 2>&1 &
app_pid=$!
...
npx playwright test tests/currency-owner.spec.ts --workers=1
docker exec "$container" psql -X -v ON_ERROR_STOP=1 -U postgres -d postgres <<'SQL'
DO $$ BEGIN
  IF (SELECT currency FROM "Organization" WHERE id = 'org-a') <> 'USD' OR (SELECT currency FROM "Organization" WHERE id = 'org-b') <> 'JPY' THEN RAISE EXCEPTION 'currency isolation failed'; END IF;
